...
def build_drill(u_meter, u_key, u_mode, u_range, u_animals, u_measures):
    ...
    for m_num in range(2, u_measures + 2):
        ...
        while beats_filled < beats_needed:
            ...
            for dur in pattern: n = note.Note(random.choice(pitches), quarterLength=dur); m.append(n)
...
# --- 4. RENDERER (SOCIALLY DISTANCED FROM CORE LOGIC) ---
def render_vexflow(score, meter_val, key_obj):
    vex_notes = []
    degree_map = SOLFEGE_MAJOR if key_obj.mode == "major" else SOLFEGE_MINOR
    dur_map = {4.0: "w", 2.0: "h", 1.5: "q", 1.0: "q", 0.75: "8", 0.5: "8", 0.25: "16"} # Simplified for stability
    ...
    for p in score.parts:
        for m in p.getElementsByClass('Measure'):
            ...
            for n in m.notesAndRests:
                ...
                syllable = degree_map.get(key_obj.getScaleDegreeFromPitch(n.pitch), "") if n.isNote else ""
                type_suffix = "r" if n.isRest else ""
                
                # Standard VexFlow Annotation logic
                note_string = f"new Vex.Flow.StaveNote({{keys: ['{p_name}'], duration: '{dur}{type_suffix}' }})"
                if syllable:
                    note_string += f".addModifier(new Vex.Flow.Annotation('{syllable}').setVerticalJustification(Vex.Flow.Annotation.VerticalJustify.BOTTOM), 0)"
                vex_notes.append(note_string)

    notes_js = ",\n            ".join(vex_notes)
    total_beats = (len(score.parts[0].getElementsByClass('Measure')) - 1) * (4 if meter_val == '4/4' else 3)
    
    vex_html = f"""
    <div id="output" style="background: white; border-radius: 10px;"></div>
    ...
    <script>
        try {{
            ...
            const voice = new Vex.Flow.Voice({{num_beats: {total_beats}, beat_value: 4}});
...
# --- 5. UI ---
st.set_page_config(page_title="SolfMaster v4.13")
st.title("🎼 SolfMaster v4.13")
with st.sidebar:
