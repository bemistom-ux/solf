...
# --- 1. MAC SYSTEM CONFIG ---
# This tells Python where to find MuseScore on your Mac
env = environment.Environment()
try:
    # Standard path for MuseScore 4
    ...
    for m_num in range(2, u_measures + 2):
        ...
        while beats_filled < beats_needed:
            ...
            for dur in pattern:
                ...
                # Add Solfege as Lyrics
                deg = k.getScaleDegreeFromPitch(n.pitch)
                n.addLyric(n.pitch.name) # We can refine this to "Do Re Mi" later
                m.append(n)
...
# --- 5. APP UI ---
st.set_page_config(page_title="SolfMaster v4.14 (Mac Edition)")
st.title("🎼 SolfMaster v4.14")
...
if 'score' in st.session_state:
    ...
    if not u_dictation or st.checkbox("Reveal Answer"):
        ...
        # MAC-NATIVE RENDERING: Creates a PNG using MuseScore
        try:
            temp_img = st.session_state['score'].write('musicxml.png')
            st.image(temp_img)
            # Cleanup the temporary file created by music21
            if os.path.exists(temp_img): os.remove(temp_img)
        except Exception as e:
            st.error(f"Mac Rendering Error: {e}")
            st.warning("Ensure MuseScore 4 is installed in your Applications folder.")
